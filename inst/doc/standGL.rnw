...
Based on the cv-curve we see that the model here fits the data slightly better than the
vanilla standardized group lasso. If we desired we could also find the coefficients of our optimal model as before.
